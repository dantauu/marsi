...
const Slides = () => {
  ...
  const { users, isLoading, currentIndex, changeCurrentIndex } = useFetchToSlide(searchParams)
  ...
  return (
    <div data-testid="slides" className="flex flex-col gap-5 px-2 pb-[90px]">
      <NotifyLastCard />
      <FilterButton />
      <SliderCard currentIndex={currentIndex} setCurrentIndex={changeCurrentIndex} data={id ? (selectedUser ? [selectedUser] : []) : users} />
    </div>
  )
}
