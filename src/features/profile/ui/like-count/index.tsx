...
export const LikeCount = ({ isPending, myLikes }: LikeCountProps) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-2 px-2 w-full h-[105px] shadow-easy rounded-[10px] bg-[#ebebeb]">
      <div
        onClick={() => navigate({ to: "/likes/my-likes" })}
        className="flex items-center flex-col gap-1 h-[34px]"
      >
        <p
          className={`font-HelveticaB leading-4 text-black ${isPending ? "text-[16px]" : "text-[17px]"}`}
        >
          Лайки от меня:
        </p>
        <>
